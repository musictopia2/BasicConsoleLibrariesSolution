...
public static class PanelExtensions
{
    ...
    public static Panel Header(this Panel panel, string text, EnumJustify? alignment = null)
    {
        if (panel is null)
        {
    ...
    /// <param name="alignment">The header alignment.</param>
    /// <returns>The same instance so that multiple calls can be chained.</returns>
    public static Panel HeaderAlignment(this Panel panel, EnumJustify alignment)
    {
        if (panel is null)
        {
            throw new ArgumentNullException(nameof(panel));
