...
public class Segment
{
    ...
    public (Segment First, Segment? Second) Split(int offset)
    {
        ...
        return (
            ...
            new Segment(Text.Substring(index), Style));
    }
    ...
    public static BasicList<SegmentLine> SplitLines(IEnumerable<Segment> segments)
    {
        ArgumentNullException.ThrowIfNull(segments);
    ...
    public static BasicList<SegmentLine> SplitLines(IEnumerable<Segment> segments, int maxWidth, int? height = null)
    {
        ArgumentNullException.ThrowIfNull(segments);

        var lines = new BasicList<SegmentLine>();
    ...
    public static BasicList<Segment> SplitOverflow(Segment segment, EnumOverflow? overflow, int maxWidth)
    {
        ArgumentNullException.ThrowIfNull(segment);

        if (segment.CellCount() <= maxWidth)
        {
            return new BasicList<Segment>(1) { segment };
        }

        // Default to folding
        overflow ??= EnumOverflow.Fold;

        var result = new BasicList<Segment>();

        if (overflow == EnumOverflow.Fold)
        {
        ...
        else if (overflow == EnumOverflow.Crop)
        {
        ...
        else if (overflow == EnumOverflow.Ellipsis)
        {
            if (Math.Max(0, maxWidth - 1) == 0)
            {
    ...
    internal static BasicList<Segment> TruncateWithEllipsis(IEnumerable<Segment> segments, int maxWidth)
    {
        ArgumentNullException.ThrowIfNull(segments);
        ...
        if (!segments.Any())
        {
            return new BasicList<Segment>(1);
        }

        var result = new BasicList<Segment>(segments);
    ...
    internal static List<string> SplitSegment(string text, int maxCellLength)
    {
        ...
        {
            if (length + SimpleCharWidth.GetWidth(ch) > maxCellLength)
            {
            ...
            length += SimpleCharWidth.GetWidth(ch);
            sb.Append(ch);
        }
