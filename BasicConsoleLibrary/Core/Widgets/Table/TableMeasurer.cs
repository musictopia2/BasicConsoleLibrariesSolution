...
internal sealed class TableMeasurer(
    BaseTable table, RenderOptions options) : TableAccessor(table, options)
{
    private const int _edgeCount = 2;
    ...
    public int GetNonColumnWidth()
    {
        ...
        var edges = hideBorder ? 0 : _edgeCount;
    ...
    public List<int> CalculateColumnWidths(int maxWidth)
    {
        ...
        if (tableWidth > maxWidth)
        {
            ...
            if (tableWidth > maxWidth)
            {
                ...
                widths = Ratio.Reduce(excessWidth, [.. widths.Select(_ => 1)], widths, widths);
                tableWidth = widths.Sum();
            }
        }
        ...
        if (tableWidth < maxWidth && Expand)
        {
            ...
            widths = [.. widths.Zip(padWidths, (a, b) => (a, b)).Select(f => f.a + f.b)];
        }
