...
{
    private static readonly Style _defaultHeadingStyle = cc1.Silver;
    private static readonly Style _defaultCaptionStyle = cc2.Gray;
    public static List<Segment> Render(TableRendererContext context, List<int> columnWidths)
    {
        ...
        {
            return new List<Segment>([new Segment("…", context.BorderStyle ?? Style.Plain)]);
        }
        ...
        foreach (var (index, isFirstRow, isLastRow, row) in context.Rows.Enumerate())
        {
            ...
            var cells = new List<BasicList<SegmentLine>>();
            ...
            if (isFirstRow && context.ShowBorder)
            {
                var separator = Aligner.Align(
                    context.Border.GetColumnRow(EnumTablePart.Top, columnWidths, context.Columns),
            ...
            {
                var textBorder = context.Border.GetColumnRow(EnumTablePart.FooterSeparator, columnWidths, context.Columns);
            ...
            if (isFirstRow && context.ShowBorder && context.ShowHeaders && context.HasRows)
            {
                var separator =
                    Aligner.Align(
                        context.Border.GetColumnRow(EnumTablePart.HeaderSeparator, columnWidths, context.Columns),
                        context.Alignment, context.MaxWidth);
                result.Add(new Segment(separator, context.BorderStyle));
                result.Add(Segment.LineBreak);
            }
                                                    ...
                                                    && (!isFirstRow || (isFirstRow && !context.ShowHeaders)) && !isLastRow)
            {
                ...
                if (!isRenderingFooter)
                {
                    var separator =
                        Aligner.Align(
                            context.Border.GetColumnRow(EnumTablePart.RowSeparator, columnWidths, context.Columns),
                            context.Alignment, context.MaxWidth);
                    result.Add(new Segment(separator, context.BorderStyle));
                    result.Add(Segment.LineBreak);
                }
            ...
            if (isLastRow && context.ShowBorder)
            {
                var separator =
                    Aligner.Align(
                        context.Border.GetColumnRow(EnumTablePart.Bottom, columnWidths, context.Columns),
                        context.Alignment, context.MaxWidth);
                result.Add(new Segment(separator, context.BorderStyle));
                result.Add(Segment.LineBreak);
            }
    ...
    private static BasicList<Segment> RenderAnnotation(TableRendererContext context, TableTitle? header,
        Style defaultStyle)
    {
        if (header == null)
        {
        ...
        var paragraph = new Text(header.Text, header.Style ?? defaultStyle)
            .Justify(EnumJustify.Center)
            .Overflow(EnumOverflow.Ellipsis);

        // Render the paragraphs
        var segments = new BasicList<Segment>();
