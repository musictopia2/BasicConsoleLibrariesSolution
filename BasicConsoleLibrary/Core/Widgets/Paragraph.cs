...
{
    private readonly BasicList<SegmentLine> _lines;
    ...
    /// </summary>
    public EnumOverflow? Overflow { get; set; }
    ...
    public Paragraph(SegmentLine line)
    {
        _lines = [];
        _lines.Add(line);
    }
    ...
    private static int GetMaxLineWidth(IEnumerable<BasicList<Segment>> lines)
    {
        int maxWidth = 0;
    ...
    protected override Measurement Measure(RenderOptions options, int maxWidth)
    {
        ...
        var min = GetMaxLineWidth(_lines);

        //var min = _lines.Max(static line => line.Max(static segment => segment.CellCount()));
    ...
    protected override IEnumerable<Segment> Render(RenderOptions options, int maxWidth)
    {
        ...
        // Justify lines
        var justification = options.Justification ?? Justification ?? EnumJustify.Left;
        if (justification != EnumJustify.Left)
        {
            foreach (var line in lines)
            {
