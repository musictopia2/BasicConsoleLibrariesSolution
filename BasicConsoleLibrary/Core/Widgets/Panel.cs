...
{
    private const int _edgeWidth = 2;
    ...
    public Panel(StyledTextBuilder builder) : this(new Markup(builder))
    {
        
    }

    public Panel(string text, Style style) : this(new Text(text, style))
    {
    ...
    {
        var edgeWidth = Border is not NoBoxBorder ? _edgeWidth : 0;
    ...
    protected override IEnumerable<Segment> Render(RenderOptions options, int maxWidth)
    {
        ...
        var edgeWidth = showBorder ? _edgeWidth : 0;
        ...
        foreach (var (_, _, last, line) in Segment.SplitLines(childSegments, innerWidth, height).Enumerate())
        {
            ...
            if (showBorder)
            {
                result.Add(new Segment(Border.GetPart(EnumBoxBorderPart.Left), borderStyle));
            }
            ...
            if (showBorder)
            {
                result.Add(new Segment(Border.GetPart(EnumBoxBorderPart.Right), borderStyle));
            }
        ...
        if (showBorder)
        {
            result.Add(new Segment(Border.GetPart(EnumBoxBorderPart.BottomLeft), borderStyle));
            result.Add(new Segment(Border.GetPart(EnumBoxBorderPart.Bottom).Repeat(panelWidth - _edgeWidth), borderStyle));
            result.Add(new Segment(Border.GetPart(EnumBoxBorderPart.BottomRight), borderStyle));
        }
        ...
        Style borderStyle, int panelWidth)
    {
        var rule = new Rule
        {
            ...
            TitleSpacing = 0,
            Title = Header?.Text,
            Justification = Header?.Justification ?? EnumJustify.Left,
        };

        // Top left border
        result.Add(new Segment(border.GetPart(EnumBoxBorderPart.TopLeft), borderStyle));
        ...
        // Top right border
        result.Add(new Segment(border.GetPart(EnumBoxBorderPart.TopRight), borderStyle));
        result.Add(Segment.LineBreak);
    }
}